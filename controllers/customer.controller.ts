...
export const getStats = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const stats = await Customer.aggregate([
            {
                $group: {
                    _id: null,
                    totalPhonesReceived: { $sum: { $size: "$phones" } },
                    totalValuation: { $sum: "$totalValuation" }
                }
            }
        ]);

        // Count phones by condition
        const conditionStats = await Customer.aggregate([
            { $unwind: "$phones" },
            {
                $group: {
                    _id: "$phones.condition",
                    count: { $sum: 1 }
                }
            }
        ]);

        const conditionBreakdown: any = {
            Excellent: 0,
            Good: 0,
            Fair: 0,
            Poor: 0
        ...
        conditionStats.forEach((s: any) => {
            if (conditionBreakdown.hasOwnProperty(s._id)) {
                conditionBreakdown[s._id] = s.count;
            }
        });
        ...
        res.status(200).json({
            totalCustomers,
            totalPhonesReceived: stats.length > 0 ? stats[0].totalPhonesReceived : 0,
            conditionBreakdown,
            totalValuation: stats.length > 0 ? stats[0].totalValuation : 0,
            recentCustomers
        });
    } catch (error: any) {
        res.status(500).json({ message: error.message });
